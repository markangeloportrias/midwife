...
(function instructorProgressOverview() {
  ...
  async function openStudent(student, count) {
    ...
    procedureFilters.slice(1).forEach(([filterKey, procedure]) => {
      ...
      const diagnosisHeader = ['delivery-handled', 'delivery-assisted'].includes(filterKey) ? 'Complete Diagnosis (Gravida, Para)' : filterKey === 'internal-exam' ? 'Internal Examination (Cervical Dilation, Effacement, BOW, Presentation and Station)' : 'Complete Diagnosis';
      section.innerHTML = `<h3 class="ip-procedure">${escape(procedure)} (${group.length})</h3><div class="ip-table-wrap ip-clinical-table-wrap clinical-form-table-container"><table class="ip-clinical-table clinical-form-table procedure-records-table prc-replica-table instructor-review-table instructor-records-table" style="min-width:1180px"><colgroup><col style="width:20.83%"><col style="width:5.48%"><col style="width:14.01%"><col style="width:7.92%"><col style="width:18.27%"><col style="width:14.01%"><col style="width:9.74%"><col style="width:9.74%"></colgroup><thead><tr><th rowspan="2">Name and Address of Patient</th><th rowspan="2">Case No.</th><th rowspan="2">${diagnosisHeader}</th><th rowspan="2">Date &amp; Time Performed</th><th rowspan="2">Full Name, Address of Facility &amp; Contact Number</th><th colspan="2">Supervised by</th><th rowspan="2">License No /<br>Expiry Date</th></tr><tr><th>Printed Name and Contact No.</th><th>Position/<br>Designation</th></tr></thead><tbody>${rowsHtml}</tbody></table></div>`;
      const procedureTable = section.querySelector('.ip-clinical-table');
      window.applyInstructorRecordTableOutline?.(procedureTable);
      procedureTable?.querySelectorAll('tbody td').forEach((cell) => {
        cell.style.setProperty('color', '#003f88', 'important');
